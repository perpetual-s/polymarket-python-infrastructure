...
| `async get_market_trades_window(market: str, start_ts: int, *, taker_only: bool = True, filter_type: Optional[str] = None, filter_amount: Optional[float] = None, max_pages: int = 4) -> Dict[str, Any]` | pages `client.data.get_trades` | `{"trades": List[Trade], "complete": bool}`; pages newest-first back to `start_ts` (keyless; `GET:/trades` 200 req/10s) | `complete=False` when the `max_pages` budget is exhausted before reaching `start_ts` (window truncated); `APIError`, `TimeoutError`, parse errors |
